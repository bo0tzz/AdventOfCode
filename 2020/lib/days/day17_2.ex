defmodule Day17_2 do
  ...
  def neighbours({_x, _y, _z, _w} = n) do
  ...
  def neighbours_keep_original({x, y, z, w}) do
    for x <- neighbours(x), y <- neighbours(y), z <- neighbours(z), w <- neighbours(w) do
      {x, y, z, w}
    end
  end
